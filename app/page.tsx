// app/login/page.tsx - UPDATE untuk menggunakan AuthContext
'use client';
...
export default function LoginPage() {
  ...
  const [rememberedAccount, setRememberedAccount] = useState<{username: string, password: string} | null>(null);
  ...
  useEffect(() => {
    ...
    if (savedAccount) {
      try {
        ...
        console.error('Error loading remembered account:', error);
      }
    }
  }, []);

  const saveRememberedAccount = (username: string, password: string) => {
    const account = { username, password, timestamp: Date.now() };
  ...
  const handleDemoLogin = async (account: { username: string, password: string }) => {
    setUsername(account.username);
    setPassword(account.password);
    ...
    setTimeout(async () => {
      ...
      try {
        const result = await authLogin(account.username, account.password);
        
        if (result.success) {
          if (rememberMe) {
            saveRememberedAccount(account.username, account.password);
          }
        ...
        setError('Demo login gagal. Coba lagi.');
        setLoading(false);
      }
    }, 300);
  };
  ...
  const getRoleByUsername = (username: string): string => {
    const roles: Record<string, string> = {
      'admin': 'Administrator',
      'string': 'Administrator', // ✅ TAMBAHKAN INI
      'viewer': 'Viewer',
      'user': 'Mahasiswa'
    };
  ...
  // ✅ AKUN DEFAULT YANG SESUAI DENGAN DATABASE
  const defaultAccounts = [
    { role: 'Admin', username: 'string', password: 'string', color: 'bg-gradient-to-r from-red-500/20 to-red-600/20' },
  ];
                ...
                {showSuccess ? (
                  ...
                  >
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      {/* Tombol Clear Remembered Account */}
                      {rememberedAccount && (
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm text-gray-700">
                                Akun tersimpan: <span className="font-semibold">{rememberedAccount.username}</span>
                              </span>
                            </div>
                            <button
                              onClick={clearRememberedAccount}
                              className="text-xs px-2 py-1 bg-red-100 hover:bg-red-200 text-red-700 rounded-md transition-colors flex items-center gap-1"
                            >
